...
//		joinResultCh := make(chan *chunk.Chunk)
...
//					jw.joiner.onMissMatch(false, outerRow, joinResult)
//
//					if joinResult.NumRows() == jw.maxChunkSize {
...
//}
//
//func (jw *mergeJoinWorker) tryToJoin(ctx context.Context, mt *mergeTask, innerIter4Row chunk.Iterator, joinResult *chunk.Chunk) bool {
//	hasMatch := false
//	for idx := 0; idx < len(mt.outerRows); {
//		outerRow := mt.outerRows[idx]
//		matched, _, err := jw.joiner.tryToMatch(outerRow, innerIter4Row, joinResult)
//		if err != nil {
//			mt.joinResultCh <- joinResult
//			joinResult = &chunk.Chunk{}
//			return false
